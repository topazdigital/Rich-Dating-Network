...
if (strcmp ($res, "VERIFIED") == 0) {
	...
	$actionText = $data[1].' days';
	$saledate = date('m/d/Y');

	$mysqli->query("INSERT INTO sales (u_id,amount,gateway,action,time,type,quantity,saledate) 
		VALUES ('".$uid."','".$payment_amount."','paypal','".$actionText."','".time()."','premium','".$data[1]."','".$saledate."')");	
	...
	if($ref != ''){
		if($refId == $uid){
			...
			$packageId = getData('config_premium','id','WHERE days = '.$days);
			$referrals = getArray('referrals','WHERE ref_status = 1','RAND()');
			if(count($referrals) > 0){
				foreach ($referrals as $r) {
		            $pos = strpos($r['ref_type'], 'purchases premium package');
		            if($pos == true) {

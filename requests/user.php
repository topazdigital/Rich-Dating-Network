...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        echo json_encode([
            "status" => "error",
            "message" => $sm['lang'][181]['text'] ?? "Invalid email"
        ]);
        exit;
    }
    ...
    if (empty($email)) {
        echo json_encode([
            "status" => "error",
            "message" => $sm['lang'][182]['text'] ?? "Email is required"
        ]);
        exit;
    }
    ...
    if ($email_check->num_rows == 0) {
        echo json_encode([
            "status" => "error",
            "message" => $sm['lang'][183]['text'] ?? "Email not found"
        ]);
        exit;
    }
    ...
    forgotMailNotification($user->name, $user->email, $link);

    echo json_encode([
        "status" => "success",
        "message" => "A reset link has been sent to your email. Please also check your spam folder."
    ]);
    exit;

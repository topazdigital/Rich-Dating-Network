...
function fastMessage(r_id){
	...
	if(plugins['chat']['creditsPerMessageEnabled'] == 'Yes'){
		...
		if(plugins['chat']['creditsPerMessageGender'] == user_info.gender){
			console.log(plugins['chat']['creditsPerMessageGender'] )
			updateCredits(user_info.id,plugins['chat']['creditsPerMessage'],1,'Credits for send chat message user_info.gender2');
			//pushNotif(data,1);
		}
		if(plugins['chat']['creditsPerMessageGender'] == allG){
			console.log(plugins['chat']['creditsPerMessageGender'] )
			updateCredits(user_info.id,plugins['chat']['creditsPerMessage'],1,'Credits for send chat message allG2');
			//pushNotif(data,1);
		}		
	}
...
$("#upload-area").dmUploader({
    ...
    if(upType == 9){ //chat photo
		...
		if(plugins['chat']['creditsPerMessageEnabled'] == 'Yes'){
			...
			if(plugins['chat']['creditsPerMessageGender'] == user_info.gender){
				updateCredits(user_info.id,plugins['chat']['creditsPerMessage'],1,'Credits for send chat message user_info.gender4');
				//pushNotif(data,1);
			}
			if(plugins['chat']['creditsPerMessageGender'] == allG){
				updateCredits(user_info.id,plugins['chat']['creditsPerMessage'],1,'Credits for send chat message al4');
				//pushNotif(data,1);
			}	
...
function chatMessage(){
	...
	$('#c-send').submit(function(e) {
		...
		if(plugins['chat']['creditsPerMessageEnabled'] == 'Yes'){
			...
			if(plugins['chat']['creditsPerMessageGender'] == user_info.gender){
				updateCredits(user_info.id,plugins['chat']['creditsPerMessage'],1,'Credits for send chat message user_info.gender');
				//pushNotif(data,1);
			}
			if(plugins['chat']['creditsPerMessageGender'] == allG){
				updateCredits(user_info.id,plugins['chat']['creditsPerMessage'],1,'Credits for send chat message allG');
				//pushNotif(data,1);
			}	
			if(plugins['chat']['transferCreditsMessageToReciever'] == 'Yes' && plugins['chat']['creditsPerMessageGender'] == user_info.gender){
                updateCredits(r_id,plugins['chat']['creditsPerMessage'],2,'Credits for message recieved');                
...
function fullHeightWall(val = 0) {
    ...
    // For credits & popularity pages, override with promo height
    if (url === 'credits' || url === 'popularity') {
        setH = $('.payment-promo').height();
    }

    const setHBody = setH + 150;

    // Live page
    if (url === 'live') {
        $('.stream').css('height', setH + 'px');
    }

    // Live Discover page
    if (url === 'live-discover') {
        $('#mainRow').css('height', setH + 'px');
    }

    // Reels page
    if (url === 'reels') {
        let reelsHeight = setH;
        if (site_theme['design_style']['val'] === 'Top-Menu') {
            reelsHeight -= 60;
        }
        $('#reelsContainer').css('height', reelsHeight + 'px');
    ...
    // Apply wall/body sizing
    if (setHBody > 450) {
        $('.wall, .real').css('height', setH + 'px');
        $('body, html').css('height', setHBody + 'px');
        $('#mainRow').css('max-height', setH + 'px');
        clearInterval(fixHeightInterval);
    } else {
        // Important fix: pass reference, not function call
        fixHeightInterval = setInterval(() => fullHeightWall(val), 100);
    }
}
...
function showProfileBio(el){
    ...
    if(edit == 0) {
        ...
        var bio = $('#userBio').html().toLowerCase().trim();

        // Normalized text for tricky checks (remove spaces, dots, underscores, hyphens)
        var normalized = bio.replace(/[\s._-]/g, "");

        // Regex patterns
        var phoneRegex = /(\+?\d[\d\s-]{7,}\d)/; 
        var phoneWordsRegex = /\b(zero|one|two|three|four|five|six|seven|eight|nine)\b/gi; 
        var emailRegex = /[a-z0-9._%+-]\s*@\s*[a-z0-9.-]+\s*\.\s*[a-z]{2,}/i; 

        // Social platforms & messaging apps
        var socialRegex = /\b(instagram|facebook|snap(chat)?|tiktok|telegram|whatsapp|wechat|twitter|x\.com|ig|fb|dm|inbox|pm|kik|viber|line|signal|onlyfans|discord|skype)\b/gi;

        // @handles
        var atHandleRegex = /(^|\s)@[a-z0-9._]{2,}/i;

        // Links & domains
        var linkRegex = /(http|www\.|\.com|\.net|\.org|\.me|\.ly|\.io|\.co|\.us|\.uk|\.ke|\.app|\.site)/i;

        // HTML/BBCode
        var htmlRegex = /<[^>]*>|(\[url|\[link|\[img|\[email|\[phone|\[contact)/i;

        // Spaced words like "w hats app"
        var spacedWordsRegex = /(i\s*g|f\s*b|w\s*hats\s*app|tele\s*gram|snap\s*chat|face\s*book|insta\s*gram)/i;

        // Normalized sneaky checks (detect in normalized string)
        var sneakyWords = /(whatsapp|telegram|facebook|instagram|snapchat|wechat|discord|onlyfans|skype|tiktok)/i;

        // Validation
        let errorMessage = "";
        if(phoneRegex.test(bio)) errorMessage = "❌ Phone numbers are not allowed in bio.";
        else if(phoneWordsRegex.test(bio)) errorMessage = "❌ Writing phone numbers in words is not allowed.";
        else if(emailRegex.test(bio)) errorMessage = "❌ Email addresses are not allowed in bio.";
        else if(socialRegex.test(bio)) errorMessage = "❌ Social media platform mentions are not allowed.";
        else if(atHandleRegex.test(bio)) errorMessage = "❌ Usernames or @handles are not allowed.";
        else if(linkRegex.test(bio)) errorMessage = "❌ External links or domains are not allowed.";
        else if(spacedWordsRegex.test(bio)) errorMessage = "❌ Spaced-out platform names are not allowed.";
        else if(htmlRegex.test(bio)) errorMessage = "❌ Custom styling, links, or HTML code is not allowed.";
        else if(sneakyWords.test(normalized)) errorMessage = "❌ Hidden mentions of external platforms are not allowed.";
        ...
        // If passes validation, continue save
        ...
        $.get(aUrl, { action: 'updateUserBio', query: query }, function(result) {
            var data = JSON.parse(result);
            var url = data['url'];
            var bioLink = bio;
            if(url != 'No'){
                var showUrl = `<br><br><a href="${data['url']}" target="_blank">${data['urlMessage']}</a>`;
